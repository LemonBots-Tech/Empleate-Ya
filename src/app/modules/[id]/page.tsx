...
export default async function ModulePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <EmployabilityShell>
      ...
      <section className="grid min-w-0 gap-7 lg:grid-cols-[0.78fr_1.22fr]">
        <aside className="relative overflow-hidden rounded-[2rem] border border-[var(--brand-border)] bg-gradient-to-br from-[var(--brand-primary-soft)] via-white to-[var(--brand-highlight-soft)] p-7 shadow-[0_25px_80px_-45px_rgba(109,40,217,0.4)] md:p-9">
          ...
          <div className="relative">
            ...
            <div className={`relative mt-10 h-48 w-48 overflow-hidden rounded-full border-4 border-white bg-gradient-to-br ${persona.themeClass} shadow-xl shadow-[var(--brand-shadow)] ring-[10px] ring-white/60 sm:h-56 sm:w-56`}>
              <Image src={persona.avatarPath} alt={`Avatar de ${persona.name}`} fill sizes="224px" className="object-cover" priority />
            </div>
        ...
        <div className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-[0_20px_70px_-45px_rgba(71,85,105,0.45)] md:p-9">
          ...
          <div className="mt-8 rounded-2xl bg-[var(--brand-primary)] p-5 text-white shadow-lg shadow-[var(--brand-shadow)] md:flex md:items-center md:justify-between">
            ...
            <Link href={skill.id === "scorex" ? "/scorex" : `/gateway?module=${skill.id}`} className="mt-4 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-extrabold text-slate-950 transition hover:bg-[var(--brand-primary-soft)] md:mt-0">

    ...
    <main className="min-h-screen bg-[var(--brand-canvas)] text-[var(--brand-ink)]">
      <header className="sticky top-0 z-20 border-b border-slate-200/70 bg-[color:var(--brand-canvas)]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-4 md:flex-row md:items-center md:justify-between">
          ...
          <nav className="flex max-w-full gap-1 overflow-x-auto pb-1 text-sm md:flex-wrap md:overflow-visible md:pb-0">
            {nav.map(([label, href]) => (
              <Link key={href} href={href} className="shrink-0 rounded-full px-3.5 py-2 font-semibold text-slate-600 transition hover:bg-white hover:text-[var(--brand-primary-strong)] hover:shadow-sm">{label}</Link>
            ))}
          </nav>
        </div>
      </header>
